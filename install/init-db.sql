-- ===============================================
-- INICIALIZACIÓN COMPLETA DE TERPENITOS GROWSHOP
-- ===============================================

-- Configurar permisos del usuario
CREATE USER IF NOT EXISTS 'terpenitos_user'@'%' IDENTIFIED BY 'terpenitos2024';
GRANT ALL PRIVILEGES ON terpenitos_growshop.* TO 'terpenitos_user'@'%';
GRANT CREATE ON *.* TO 'terpenitos_user'@'%';
FLUSH PRIVILEGES;

-- Usar la base de datos
USE terpenitos_growshop;
...
-- Password hash para 'admin123': $2y$10$7rZJQQqxJJXLKRXgtY0YmeB5vYPCJdpgXNiC7DPIAiLftG1QVJvx2
INSERT INTO users (name, email, phone, password, is_admin) VALUES 
('Administrador Principal', 'admin@terpenitos.com', '+54 9 11 1111-1111', '$2y$10$7rZJQQqxJJXLKRXgtY0YmeB5vYPCJdpgXNiC7DPIAiLftG1QVJvx2', 1);

-- Usuario regular de prueba (email: user@test.com, password: user123)
INSERT INTO users (name, email, phone, password, is_admin) VALUES 
('Usuario de Prueba', 'user@test.com', '+54 9 11 2222-2222', '$2y$10$7rZJQQqxJJXLKRXgtY0YmeB5vYPCJdpgXNiC7DPIAiLftG1QVJvx2', 0);

-- Insertar direcciones de ejemplo
INSERT INTO user_addresses (user_id, alias, address_line1, city, postal_code, is_default) VALUES 
(2, 'Casa', 'Libertad 123, Buenos Aires', 'Buenos Aires', '1000', TRUE);

-- Insertar un pedido de ejemplo
INSERT INTO orders (user_id, status, total, shipping_address, phone, notes) VALUES 
(2, 'Entregado', 5000.00, 'Libertad 123, Buenos Aires (1000)', '+54 9 11 2222-2222', 'Entregar en horario laboral');
